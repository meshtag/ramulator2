...
private:
      ...
      if (m_dram->m_command_meta(req_it->command).is_closing) {
        ...
        for (auto _it = m_active_buffer.begin(); _it != m_active_buffer.end();
          ...
          if (rowgroup == _it_rowgroup) {
            // Invalidate this scheduling outcome if we are to interrupt a
            // request in the active buffer
            request_found = false;
          }
        }
      }
  ...
  bool clear() override {
    ...
    if (m_active_buffer.size())
      no_pending = false;
    if (m_priority_buffer.size())
      no_pending = false;
    if (m_read_buffer.size())
      no_pending = false;
    if (m_write_buffer.size())
      no_pending = false;
    if (pending.size())
