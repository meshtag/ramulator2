...
public:
      ...
      LUT(m_commands, m_levels,
          {
              ...
              {"SASEL", "row"},
              {"SARD", "column"},
              {"SAWR", "column"}, // PIM: Subarray-level commands; PIMOp depends
                                  // on PIM tech.
              {"BKRD", "column"},
              {"BKWR", "column"}, // PIM: Bank-level commands
          });
  ...
  inline static const ImplLUT m_command_meta = LUT<DRAMCommandMeta>(
      ...
      {// open?   close?   access?  refresh?
       ...
       {"PIMOp",
        {false, true, false,
         false}}, // PIMOp stores the result row back to memory using PRE
...
private:
  void set_organization() {
    ...
    // Sanity check: is the calculated channel density the same as the provided
    // one?
...
#define V(timing) (m_timing_vals(timing))
    populate_timingcons(
        this, {
                  ...
                  /*** Bank ***/ // comment these constraints because of
                                 // subarray-level parallelism
                  // {.level = "bank", .preceding = {"ACT"}, .following =
                  // {"ACT"}, .latency = V("nRC")},
                  {.level = "bank",
                   .preceding = {"ACT"},
                  ...
                  // {.level = "bank", .preceding = {"ACT"}, .following =
                  // {"PRE"}, .latency = V("nRAS")},
                  // {.level = "bank", .preceding = {"PRE"}, .following =
                  // {"ACT"}, .latency = V("nRP")},
                  {.level = "bank",
                   .preceding = {"RD", "BKRD"},
                   .following = {"PRE"},
